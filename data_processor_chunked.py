...
class DataProcessor:
    ...
    def extract_hof_enhancers(self, metadata_df, peak_data):
        ...
        if metadata_df is not None and not metadata_df.empty:
            # Get one representative row per enhancer from metadata with all important columns
            metadata_columns = ['enhancer_id', 'cargo', 'experiment', 'proximal_gene', 'GC delivered']
            # Only include columns that actually exist
            available_columns = [col for col in metadata_columns if col in metadata_df.columns]
            
            print(f"Available metadata columns: {available_columns}")
            print(f"Metadata shape before groupby: {metadata_df.shape}")
            
            # Group by enhancer_id and take first non-null value for each column
            metadata_summary = metadata_df.groupby('enhancer_id').first().reset_index()
            print(f"Metadata shape after groupby: {metadata_summary.shape}")
            print(f"Sample metadata record: {metadata_summary.iloc[0][available_columns].to_dict()}")
            
            # Merge with HOF enhancers
            hof_df = hof_df.merge(metadata_summary[available_columns], 
                                 on='enhancer_id', how='left')
            print(f"HOF enhancers shape after merge: {hof_df.shape}")
            
            # Check if merge was successful
            for col in ['cargo', 'experiment', 'proximal_gene']:
                if col in hof_df.columns:
                    non_null_count = hof_df[col].notna().sum()
                    print(f"  {col}: {non_null_count} non-null values")
                    if non_null_count > 0:
                        print(f"    Sample {col} values: {hof_df[col].dropna().unique()[:3]}")
            
            print(f"Sample merged HOF enhancer: {hof_df.iloc[0].to_dict()}")
