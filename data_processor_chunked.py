...
class DataProcessor:
    ...
    def __init__(self):
        self.data_dir = "."  # Look in current directory instead of data_chunks
    ...
    def extract_hof_enhancers(self, metadata_df, peak_data):
        ...
        # Create base enhancer information from peak data (only for HOF enhancers)
        hof_enhancers = []
        for enhancer_id in hof_enhancer_ids:
            ...
            if not enhancer_data.empty:
                ...
                print(f"Warning: HOF enhancer {enhancer_id} not found in peak data")
        
        hof_df = pd.DataFrame(hof_enhancers)
        
        # Verify the merge worked
        if not hof_df.empty:
            print(f"HOF enhancers created with shape: {hof_df.shape}")
            print(f"HOF enhancers columns: {list(hof_df.columns)}")
            
            # Check if metadata was successfully integrated
            if 'cargo' in hof_df.columns:
                cargo_non_null = hof_df['cargo'].notna().sum()
                cargo_non_na = (hof_df['cargo'] != 'N/A').sum()
                print(f"Cargo: {cargo_non_null} non-null, {cargo_non_na} non-N/A values")
                if cargo_non_na > 0:
                    print(f"  Sample cargo values: {hof_df[hof_df['cargo'] != 'N/A']['cargo'].unique()[:3]}")
            
            if 'experiment' in hof_df.columns:
                exp_non_null = hof_df['experiment'].notna().sum()
                exp_non_na = (hof_df['experiment'] != 'N/A').sum()
                print(f"Experiment: {exp_non_null} non-null, {exp_non_na} non-N/A values")
                if exp_non_na > 0:
                    print(f"  Sample experiment values: {hof_df[hof_df['experiment'] != 'N/A']['experiment'].unique()[:3]}")
            
            print(f"Sample HOF enhancer record: {hof_df.iloc[0].to_dict()}")
        
        print(f"Extracted {len(hof_df)} Hall of Fame enhancers with integrated metadata")
        return hof_df
