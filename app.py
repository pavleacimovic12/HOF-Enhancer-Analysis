...
class GitHubDataProcessor(DataProcessor):
    ...
    def __init__(self):
        super().__init__()
        # Override data paths for GitHub structure - files are in current directory
        self.data_dir = "."
        
    def load_metadata(self):
        """Load enhancer metadata from feather file in current directory"""
        try:
            metadata_path = "Enhancer_and_experiment_metadata_1751579195077.feather"
    ...
    def load_peak_data(self):
        """Load and combine chunked CSV files from current directory"""
        try:
            import glob
            ...
            for pattern in chunk_patterns:
                ...
                if chunk_files:
                    st.info(f"Loading {len(chunk_files)} chunks for pattern {pattern}")
                    
                    for chunk_file in chunk_files:
                        df = pd.read_csv(chunk_file)
                        all_chunks.append(df)
                        st.info(f"  Loaded {os.path.basename(chunk_file)}: {len(df):,} rows")
            
            if not all_chunks:
                st.error("No chunk files found in current directory")
                ...
                if current_csv_files:
                    st.info(f"CSV files found: {current_csv_files[:10]}")  # Show first 10
                return None
            
            # Combine all chunks
            combined_df = pd.concat(all_chunks, ignore_index=True)
            st.success(f"Combined all chunks: {len(combined_df):,} total rows")
            
            # Remove any duplicate rows
            original_length = len(combined_df)
            combined_df = combined_df.drop_duplicates()
            if len(combined_df) < original_length:
                st.info(f"Removed {original_length - len(combined_df):,} duplicate rows")
...
enhancer_ids_to_include = set(hof_enhancers['enhancer_id'].unique()) if hof_enhancers is not None else set()
